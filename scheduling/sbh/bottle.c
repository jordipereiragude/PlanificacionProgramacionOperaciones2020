...
void twiddle (sequencetype *seq,motype *mo,int *make);
void bottler (sequencetype *seq,motype *mo,int try_count,int *try_values);
...
void bottler (seq, mo, try_count, try_values)
sequencetype *seq;
motype *mo;
...
char *fname;
{
    ...
    for(i=0;i<njobs;i++) {
      for(j=0;j<nmachs;j++) {
        fscanf(in,"%d ",&ptime);
        printf("%d ",ptime);
        job[i].proctime[job[i].order[j]]=ptime;
      }
      printf("\n");
    }
        fclose (in);
